...
test.describe('Homepage interactions', () => {
    ...
    test('Djinn supports typed audio replies, volume, and microphone switching', async ({ page, browserName }, testInfo) => {
        ...
        const mute = page.locator('[data-djinn-mute]');
        await mute.click();
        await expect(volume).toHaveValue('0');
        await expect(mute).toHaveAttribute('aria-pressed', 'true');
        await expect(page.locator('[data-djinn-volume-cross]')).toBeVisible();
        await expect(page.locator('[data-djinn-volume-waves]')).toBeHidden();
        await mute.click();
        await expect(volume).toHaveValue('35');
        await volume.evaluate((element) => { (element as HTMLInputElement).value = '0'; element.dispatchEvent(new Event('input', { bubbles: true })); });
        await expect(mute).toHaveAttribute('aria-pressed', 'true');
        await mute.click();
        await expect(volume).toHaveValue('35');
        await expect(page.locator('[data-djinn-volume-cross]')).toBeHidden();
        ...
        await page.evaluate(() => { (window as any).__savedSocket = (window as any).__djinnSocket; });
        await page.locator('[data-home-header] .nav-brand').dispatchEvent('pointerdown');
        await expect(page.locator('[data-djinn-response]')).toBeHidden();
        expect(await page.evaluate(() => (window as any).__djinnAudioContext.gains[0].gain.value)).toBe(0);
        expect(await page.evaluate(() => (window as any).__djinnSocket.readyState)).toBe(1);
        expect(await page.evaluate(() => (window as any).__djinnSocket.sent.some((m: any) => m.type === 'end'))).toBe(false);
        await page.evaluate(() => {
            const socket = (window as any).__djinnSocket;
            socket.emit({ type: 'audio_start', turnId: 2, sequence: 0, text: 'The museum can use the same approach.', sampleRate: 24000 });
            socket.emitAudio(2000);
            (window as any).__djinnAudioContext.currentTime = 4.3;
            socket.emit({ type: 'audio_end', turnId: 2, sequence: 0 });
            socket.emit({ type: 'turn_complete', turnId: 2 });
        });
        await expect(answer).toHaveText('The museum can use the same approach.');
        await expect(answer).not.toHaveClass(/djinn-message--active/);
        expect(await page.evaluate(() => (window as any).__djinnSocket.sent.some((m: any) => m.type === 'playback_ack' && m.turnId === 2))).toBe(false);
        await page.locator('[data-djinn-keyboard]').click();
        await expect(page.locator('[data-djinn-message="visitor"]')).toHaveCount(2);
        expect(await page.evaluate(() => (window as any).__djinnAudioContext.gains[0].gain.value)).toBe(0.35);
        expect(await page.evaluate(() => (window as any).__djinnSocket === (window as any).__savedSocket)).toBe(true);
        await expect(answer).toHaveText('The museum can use the same approach.');
        await page.locator('[data-djinn-input]').fill('What did we just discuss?');
        await page.locator('[data-djinn-input]').press('Enter');
        await expect(page.locator('[data-djinn-message="visitor"]')).toHaveCount(3);
        await expect(page.locator('[data-djinn-message="notice"]')).toHaveCount(0);
